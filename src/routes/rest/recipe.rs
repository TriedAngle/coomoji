...
pub async fn by_operation(pool: web::Data<PgPool>, web::Path(id): web::Path<i32>) -> Result<HttpResponse, Error> {
    let item = Recipe::by_operation(id, &pool).await.unwrap();
    Ok(HttpResponse::Ok().json(item))
}

#[get("/api/recipes/by-outcome/{id}")]
pub async fn by_outcome(pool: web::Data<PgPool>, web::Path(id): web::Path<i32>) -> Result<HttpResponse, Error> {
    let item = Recipe::by_outcome(id, &pool).await.unwrap();
    Ok(HttpResponse::Ok().json(item))
}

#[get("/api/recipes/by-components")]
pub async fn by_components(pool: web::Data<PgPool>, web::Json(components): web::Json<Vec<i32>>) -> Result<HttpResponse, Error> {
...
#[post("/api/recipes")]
pub async fn new(pool: web::Data<PgPool>, web::Json(item): web::Json<NewRecipe>) -> Result<HttpResponse, Error> {
    let item = Recipe::create(item, &pool).await.unwrap();
    Ok(HttpResponse::Ok().json(item))
}

#[patch("/api/recipes/{id}")]
pub async fn update(pool: web::Data<PgPool>, web::Path(id): web::Path<i32>, web::Json(item): web::Json<NewRecipe>) -> Result<HttpResponse, Error> {
