...
pub async fn by_emoji(pool: web::Data<PgPool>, web::Path(id): web::Path<i32>) -> Result<HttpResponse, Error> {
    let item = Operation::by_emoji(id, &pool).await.unwrap();
    Ok(HttpResponse::Ok().json(item))
}

#[post("/api/operations")]
pub async fn new(pool: web::Data<PgPool>, web::Json(item): web::Json<NewOperation>) -> Result<HttpResponse, Error> {
    let item = Operation::create(item, &pool).await.unwrap();
    Ok(HttpResponse::Ok().json(item))
}

#[patch("/api/operations/{id}")]
pub async fn update(pool: web::Data<PgPool>, web::Path(id): web::Path<i32>, web::Json(item): web::Json<NewOperation>) -> Result<HttpResponse, Error> {
